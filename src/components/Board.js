    ...
    render() {
        ...
        for (let i = 0; i < 10; i++) {
            squares[i].map((singleSquare) => {
                return mappedBoard.push(<Square key={singleSquare.key} coordinate={singleSquare.key} color={singleSquare.color} onClick={this.clickItem} active={this.props.state.squares.active}/>)
            })
        }
